  ...
  const submit = async () => {
    ...
    try {
      ...
      if (!submittedRef.current) {
        ...
        // Lưu attempt và đánh dấu hoàn thành
        await Promise.all([
        ...
        setSaved(true);
        // Gọi callback để đồng bộ readSet trên frontend
        onMarkAllRead?.();
      }
    } catch (e) {
      ...
      {result && (
        <div className="mt-6 space-y-4 animate-in fade-in slide-in-from-bottom-4">
          ...
          {result.feedback && (
            <div>
              <h4 className="mb-2 flex items-center gap-2 text-sm font-semibold">
                <Star className="h-4 w-4 text-amber-500" /> Nhận xét của AI
              </h4>
          ...
          {result.optimized && result.optimized !== code && (
            <div>
              <h4 className="mb-2 flex items-center gap-2 text-sm font-semibold">
                <Sparkles className="h-4 w-4 text-primary" /> Bản tối ưu gợi ý
              </h4>
