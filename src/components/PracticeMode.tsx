...
export function PracticeMode({
  ...
  // resumeBanner state removed — auto-resume thay thế hoàn toàn
  ...
  /* ---------- init + auto-resume when lesson changes ----------
   * Gộp reset & load: khi lesson đổi (hoặc mount), nạp tiến độ
   * đã lưu trong localStorage và áp dụng trực tiếp — KHÔNG cần
   * banner xác nhận. Nếu không có cache thì reset về mặc định. */
  useEffect(() => {
    // Tạm dừng autosave trong lúc khôi phục để tránh ghi đè cache
    ...
    const hasProgress =
      !!persisted &&
      !!persisted.results &&
      Object.keys(persisted.results).length > 0;

    if (hasProgress && persisted) {
      setCurrentEx(persisted.currentEx ?? 0);
      setResults(persisted.results ?? {});
      setXp(persisted.xp ?? 0);
      setStreak(persisted.streak ?? 0);
      setBestStreak(persisted.bestStreak ?? 0);
      setShields(persisted.shields ?? 0);
      setBookmarks(new Set(persisted.bookmarks ?? []));
      setTimerEnabled(!!persisted.timerEnabled);
      setChallengeMode(!!persisted.challengeMode);
      // Ưu tiên mode đã lưu (vd: user đang practice thì quay lại vẫn practice)
      setMode(persisted.mode ?? initialMode);
      setElapsed(0);
      setExStart(Date.now());

      // Toast nhỏ báo đã khôi phục
      const id = ++toastIdRef.current;
      const doneCount = Object.keys(persisted.results ?? {}).length;
      setToasts((t) => [
        ...t,
        {
          id,
          text: `💾 Đã khôi phục tiến độ (${doneCount} câu · ${persisted.xp ?? 0} XP)`,
          kind: "info",
        },
      ]);
      setTimeout(
    ...
    // Kích hoạt autosave sau khi đã khôi phục/reset xong
  ...
  useEffect(() => {
    ...
    // Tránh ghi nhầm key khi lesson vừa đổi (state cũ còn tồn tại)
  ...
  /* ---------- clear cache thủ công (vd: nút "Bắt đầu lại") ---------- */
  const discardProgress = useCallback(() => {
    clearPersisted(storageKey);
  }, [storageKey]);
  void discardProgress; // giữ helper export-ready, tránh unused-warning
  ...
  return (
    <div className="space-y-4">
      ...
      {/* Resume banner đã được thay bằng auto-resume + toast */}
