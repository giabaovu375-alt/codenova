...
function RoadmapPage() {
  ...
  // ─── BUG FIX: dùng ref để track mounted state, tránh setState sau unmount
  ...
  const fetchData = useCallback(
    async (silent = false) => {
      try {
        // BUG FIX: luôn reset loading khi không silent để skeleton hiện đúng
        if (!silent) {
          setLoading(true);
          setError(null);
        }
        ...
        // BUG FIX: chỉ setState nếu component vẫn còn mounted
  ...
  // BUG FIX: reset state hoàn toàn khi fetchData thay đổi (user thay đổi / navigate lại)
  useEffect(() => {
    setLessons([]);
    setProgress([]);
    setAttempts([]);
  ...
  // ─── Confetti + toast khi hoàn thành bài ────────────────────────────────
  useEffect(() => {
  ...
  // BUG FIX: khai báo trước useEffect dùng nó
  const [activeRoadmapId, setActiveRoadmapId] = useState<string>("");
  const [searchTerm, setSearchTerm] = useState("");
  const [difficultyFilter, setDifficultyFilter] = useState<DifficultyFilter>("all");
  const [sortMode, setSortMode] = useState<SortMode>("default");

  // BUG FIX: bỏ activeRoadmapId khỏi dependency để tránh vòng lặp
  useEffect(() => {
