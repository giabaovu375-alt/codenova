...
// ────────────────────────────────────────────────────────────────────────────
// 🚀 CodeNova — Roadmap Page (PREMIUM EDITION)
//
// Highlights:
//  • 100% bulletproof progress sync: progress-updated event + storage event +
//    visibilitychange + window focus + optimistic refresh-on-mount.
//  • Level system (XP → Level), daily goal ring, 7-day heatmap, achievements.
//  • Animated streak flame, circular progress, confetti on roadmap complete.
//  • Difficulty filter chips, sort, search, keyboard nav (j/k, 1-9).
//  • Premium gradient header, glassmorphism cards, skeleton loaders.
//  • Module timeline with score badge, locked tooltip, smooth entry animations.
//  • Sticky "Continue learning" CTA on mobile.
//  • Toast on lesson completion.
// ────────────────────────────────────────────────────────────────────────────
...
interface Roadmap {
  ...
  difficulty: string;
  difficultyLevel: 1 | 2 | 3; // for sorting/filtering
  estimatedTime: string;
  modules: Module[];
...
const buildRoadmaps = (
  ...
  const applyLocking = (modules: Module[]) => {
    for (let i = 1; i < modules.length; i++) {
      if (!modules[i - 1].completed) modules[i].locked = true;
    }
    return modules;
  };
  ...
  push(
    "python",
    "Python Developer",
    "Từ cú pháp cơ bản đến OOP và AI/ML",
    Code,
    "Cơ bản - Trung cấp",
    pythonModules,
  );
  ...
  push(
    "js-mastery",
    "JavaScript Mastery",
    "Nâng cao kỹ năng JavaScript hiện đại",
    Zap,
    "Trung cấp - Nâng cao",
    jsModules,
  );
  ...
  push(
    "java",
    "Java Developer",
    "Lập trình hướng đối tượng và ứng dụng",
    BookOpen,
    "Cơ bản - Trung cấp",
    javaModules,
  );
...
function RoadmapPage() {
  ...
  // ─── Realtime sync: nhiều layer để bảo đảm 100% cập nhật ───────────────
  useEffect(() => {
    const reload = () => fetchData(true);

    // 1. Custom event từ progress-store
    window.addEventListener("progress-updated", reload);

    // 2. progressEvents emitter (nếu store expose)
    ...
    // 3. Storage event (sync giữa các tab)
    ...
    // 4. Visibility change (user quay lại tab → refetch)
    const onVisibility = () => {
  ...
  // ─── Detect completion → confetti + toast ───────────────────────────────
  useEffect(() => {
  ...
  const stats = useMemo(() => {
    ...
    // Combine for streak (any activity counts)
  ...
  const achievements = useMemo(
    () => [
      ...
      },
      {
        ...
        unlocked: roadmaps.some(
          (r) => calculateProgress(r.modules).percent === 100,
        ),
        color: "emerald",
      },
    ],
  ...
  // ─── Keyboard nav: j/k between roadmaps, 1-9 jump ───────────────────────
  useEffect(() => {
      ...
      <CodeNovaLayout>
        <div className="flex flex-col items-center justify-center py-20 text-center">
          ...
          <p className="text-muted-foreground">
            Chưa có bài học nào để tạo lộ trình.
          </p>
        </div>
      </CodeNovaLayout>
    );
    ...
    <CodeNovaLayout>
      ...
      <div className="mx-auto max-w-6xl px-4 pt-10 pb-24">
              ...
              <div className="flex items-center gap-4">
                ...
                <div className="space-y-1.5">
                  <div className="flex items-center gap-1.5 rounded-full border border-orange-500/30 bg-orange-500/10 px-3 py-1 text-xs font-semibold text-orange-500">
                    <Flame
                      className={`h-3.5 w-3.5 ${stats.streak > 0 ? "animate-pulse" : ""}`}
                    />
        ...
        {activeRoadmap && (
          <div className="rounded-2xl border border-border bg-card/60 p-4 md:p-6 backdrop-blur">
            <div className="flex items-center gap-4 mb-6">
              <div
                className={`flex h-14 w-14 items-center justify-center rounded-2xl bg-gradient-to-br ${activeRoadmap.accent} text-white shadow-lg`}
              >
            ...
            <div className="grid md:grid-cols-3 gap-6">
              ...
              <div className="md:col-span-2">
                <h3 className="text-lg font-semibold text-foreground mb-4 flex items-center gap-2">
                  <BookOpen className="h-5 w-5 text-primary" /> Lộ trình chi
                  tiết
                </h3>
              ...
              <div className="space-y-5">
                ...
                <div className="rounded-xl border border-border bg-card/80 p-4">
                  ...
                  <div className="mt-2 flex justify-between text-xs text-muted-foreground">
                    <span>
                      {calculateProgress(activeRoadmap.modules).completed} hoàn
                      thành
                    </span>
                    <span>
                      {calculateProgress(activeRoadmap.modules).total} tổng
                    </span>
                  </div>
                </div>
                ...
                {recentActivity.length > 0 && (
                  <div className="rounded-xl border border-border bg-card/80 p-4">
                    <h4 className="text-sm font-semibold text-foreground mb-3 flex items-center gap-1.5">
                      <CalendarDays className="h-4 w-4 text-emerald-500" /> Gần
                      đây
                    </h4>
        ...
        <div className="mt-8 text-center text-[11px] text-muted-foreground">
          ...
          <kbd className="rounded bg-secondary px-1.5 py-0.5">k</kbd> để chuyển
          lộ trình, hoặc{" "}
          <kbd className="rounded bg-secondary px-1.5 py-0.5">1-9</kbd> để nhảy
          nhanh.
        </div>
      </div>
    </CodeNovaLayout>
