// ai.ts — Ultimate Multi-AI Provider Hub (10 providers)
...
export function suggestModelForLanguage(provider: ProviderId, language: string): string {
  ...
  if (!info) return info?.defaultModel ?? "";
...
// OpenAI-compatible API (dùng cho OpenAI, DeepSeek, Groq, OpenRouter, Mistral, Together, Cerebras)
...
async function callHuggingFaceAPI(key: string, model: string, messages: Msg[]): Promise<string> {
  ...
  if (!res.ok) await readError(res, "Hugging Face");
  const data = await res.json();
...
export async function chat(
  ...
  switch (info.apiFormat) {
    ...
    case "gemini":
      return callGeminiAPI(key, model, messages);
    case "huggingface":
      return callHuggingFaceAPI(key, model, messages);
    default:
